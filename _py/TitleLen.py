...
title_files = [
    ...
    for f in fnmatch.filter(files, 'Title_*.txt')
]

for files in title_files:
...
if len(sys.argv) == 3:
    ...
    FSWP = OrderedDict((key, value) for key, value in FSs.items() if value >= 33)
    for e, s in FSWP.items():
        counterr += 1
        print(u"Title Name '{}' is too long: {}".format(e, s))
